...
class ImageFeed(models.Model):
    user = models.ForeignKey(settings.AUTH_USER_MODEL,
                             on_delete=models.CASCADE)  # Пользователь, загрузивший изображение
    ...
    processed_image = models.ImageField(upload_to='processed_images/', null=True,
                                        blank=True)  # Обработанное изображение
...
class DetectedObject(models.Model):
    image_feed = models.ForeignKey(ImageFeed, related_name='detected_objects',
                                   on_delete=models.CASCADE)  # Связь с ImageFeed
...
class DescribedImage(models.Model):
    image_feed = models.ForeignKey(ImageFeed, related_name='described_image',
                                   on_delete=models.CASCADE)  # Связь с ImageFeed
